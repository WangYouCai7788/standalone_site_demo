      ...
      <div class="container">
            ...
            <div v-for="inquiry in inquiries" :key="inquiry.id" class="inquiry-card card">
              ...
              <div class="inquiry-body">
                <div class="inquiry-meta">
                  ...
                  <span><strong>{{ isZh ? '产品' : 'Product' }}:</strong> {{ inquiry.product }}</span>
                  <span v-if="inquiry.quantity"><strong>{{ isZh ? '数量' : 'Qty' }}:</strong> {{ inquiry.quantity }}</span>
                </div>
        ...
        <div v-if="tab === 'products'">
          <div class="admin-toolbar">
            <button class="btn btn-primary btn-sm" @click="showAddForm = true">+ {{ isZh ? '添加产品' : 'Add Product' }}</button>
          </div>
            ...
            <form @submit.prevent="addProduct">
              <div class="form-row">
                <div class="form-group">
                  <label>{{ isZh ? '产品名称' : 'Product Name' }} *</label>
                  <input v-model="newProduct.name" class="form-input" required />
                </div>
                <div class="form-group">
                  <label>{{ isZh ? '产品名称（中文）' : 'Product Name (Chinese)' }}</label>
                  <input v-model="newProduct.nameCn" class="form-input" />
                </div>
              </div>
              <div class="form-row">
                <div class="form-group">
                  <label>{{ isZh ? '分类' : 'Category' }}</label>
              ...
              <div class="form-group">
                <label>{{ isZh ? '产品描述' : 'Description' }}</label>
                <textarea v-model="newProduct.description" class="form-textarea" required></textarea>
              </div>
              <div class="form-actions">
                <button type="submit" class="btn btn-primary btn-sm">{{ isZh ? '保存产品' : 'Save Product' }}</button>
                <button type="button" class="btn btn-outline btn-sm" @click="showAddForm = false">{{ isZh ? '取消' : 'Cancel' }}</button>
              </div>
            </form>
          </div>
          <div class="grid-4 mt-2">
            <div v-for="product in sortedProducts" :key="product.id" class="admin-product-card card">
              ...
              <div class="admin-product-info">
                ...
                <h4>{{ localized(product, 'name', 'nameCn') }}</h4>
                <p class="text-light">FOB: {{ product.fobPrice }} | {{ isZh ? '最小起订量' : 'MOQ' }}: {{ product.moq }}{{ product.moqUnit }}</p>
              </div>
            </div>
          </div>
        </div>
...
function loadInquiries() {
  try { inquiries.value = JSON.parse(localStorage.getItem('inquiries') || '[]') }
...
function loadProducts() {
  try { customProducts.value = JSON.parse(localStorage.getItem('customProducts') || '[]') }
  catch { customProducts.value = [] }
}
loadProducts()

const sortedProducts = computed(() => [...defaultProducts, ...customProducts.value])
...
function addProduct() {
  const item = { id: Date.now(), ...newProduct.value, moq: Number(newProduct.value.moq), features: [], specs: {} }
...
<style scoped>
.page-hero { background: var(--bg-gray); padding: 120px 0 40px; border-bottom: 1px solid var(--border); }
.page-hero h1 { font-size: 32px; font-weight: 700; margin-bottom: 8px; }
.page-hero p { color: var(--text-light); font-size: 15px; }
.admin-nav { display: flex; gap: 8px; margin-bottom: 32px; }
.filter-btn { display: flex; align-items: center; gap: 8px; padding: 10px 24px; border: 1px solid var(--border); background: white; border-radius: var(--radius); font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.2s; color: var(--text); }
...
.inquiry-card { border-left: 3px solid var(--primary); }
.inquiry-header { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px 8px; }
.inquiry-date { font-size: 12px; color: var(--text-light); }
.inquiry-body { padding: 0 20px 16px; }
.inquiry-meta { display: flex; flex-wrap: wrap; gap: 16px; font-size: 13px; color: var(--text-light); margin-bottom: 12px; }
.inquiry-message { font-size: 14px; line-height: 1.7; background: var(--bg-gray); padding: 12px 16px; border-radius: 6px; }
...
.admin-product-info p { font-size: 12px; }
@media (max-width: 768px) { .form-row { grid-template-columns: 1fr; } .page-hero h1 { font-size: 26px; } }
</style>
